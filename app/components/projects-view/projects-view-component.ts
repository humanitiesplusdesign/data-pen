...
export class ProjectsViewComponentController implements angular.IComponentController {
  ...
  constructor(private projectService: ProjectService, socialAuthService: SocialAuthService, $localStorage: any) {
    ...
    if (socialAuthService.isLoggedIn() && !projectSources.some(s => s.id === 'Personal projects')) {
      ...
      projectSources.unshift(new ProjectSourceInfo('Personal projects', 'http://ldf.fi/fibra/sparql', 'http://ldf.fi/fibra/sparql', 'http://ldf.fi/fibra/sparql', 'http://ldf.fi/fibra/user/' + uid + '/projects/', 'http://ldf.fi/fibra/fusekiEndpointWithTextIndexAndSecoFunctions'))
    }
