...
export class ItemsService {
  ...
  public getAllItems(): angular.IPromise<{}[]> {

    return this.$q.resolve(
      this.testData.filter((item) => {
        return !item.id.startsWith('http://www.w3.org/') &&
          !item.id.startsWith('http://xmlns.com/') &&
          !item.id.startsWith('http://www.cidoc-crm.org/') &&
          !item.id.startsWith('http://purl.org/')
      })
    )
  }
...
angular.module('fibra.services.items-service', [])
  .config(($provide) => {
    $provide.service('itemsService', ItemsService)
  })
